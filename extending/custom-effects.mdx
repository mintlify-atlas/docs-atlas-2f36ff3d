---
title: "Adding Custom Visual Effects to COI Client"
sidebarTitle: "Custom Effects"
description: "Implement the VisualEffect interface and register your effect in EffectManager to add new screen-space effects to COI Client."
---
...
<Note>
  `render()` receives a `GuiGraphicsExtractor` — a COI Client extension of Minecraft's `GuiGraphics` — along with pre-computed `screenWidth` and `screenHeight` values. Use `ctx.fill()`, `ctx.fillGradient()`, and similar helpers to draw screen-space geometry.
</Note>
...
- **Photosensitive effects** — if your effect involves rapid flashing or strobing, add its `ID` to `EffectManager.PHOTOSENSITIVE_EFFECTS` so it is skipped when the player has enabled `epilepsyMode` in `HudConfig`.
