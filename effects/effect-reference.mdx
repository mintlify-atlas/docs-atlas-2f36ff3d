...
<Accordion title="whispers">
  ...
  **Default phrase pool:**
  > "they can see you" · "don't look back" · "it knows your name" · "run" · "the door is open" · "help me" · "it's behind you"
...
<Accordion title="tunnel">
  A circular vignette closes inward from all edges, leaving only a shrinking oval of visibility at the screen center. Rendered via scanline fill (~80 draw calls per frame). Good for: exhaustion, confusion, extreme madness, near-death.

  <Warning>
    Due to the scanline rendering approach (~80 draw calls per frame), `tunnel` is the most GPU-intensive effect. Avoid running it simultaneously with other heavy effects for extended durations.
  </Warning>
