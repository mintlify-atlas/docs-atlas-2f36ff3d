...
| White Tower | Blinding pure light swirling with infinite knowledge symbols |
...
| Hanged Man | Massive shadow bound in grotesque chains and an upside-down cross |
...
| Twilight Giant | Colossal armored giant glowing with the dim light of impending sunset |
| Red Priest | Towering being of raging crimson flames shaped like an armored warlord |
...
| Wheel of Fortune | Multi-layered rotating wheel engraved with divination cards and fate threads |
| Chained Man | Distorted humanoid in rust-iron chains straining to burst into beast form |
| Abomination | Ever-changing mass of black flesh, mouths, limbs, and maddening tentacles |
| Justiciar | Golden balance scale with an armored judge silhouette enforcing absolute order |
| Black Emperor | Shadow-imperial figure in a spiked crown warping laws and physical rules |

## How Transformations Work

<Steps>
  <Step title="Server sends MythicalFormPayload">
    The server dispatches a `coi-client:mythical_form` plugin message containing the target player's UUID and a params string formatted as `<pathwayName>:<tier>:<action>` (e.g., `fool:5:start`).
  </Step>
...
REGISTERED_FORMS.put(name,                          form); // "white tower"
REGISTERED_FORMS.put(name.replace(" ", ""),         form); // "whitetower"
REGISTERED_FORMS.put(name.replace("_", ""),         form); // "whitetower" (from underscore source)
REGISTERED_FORMS.put(name.replace(" ", "_"),        form); // "white_tower"
REGISTERED_FORMS.put(name.replace("_", " "),        form); // "white tower" (from underscore source)
```

All keys are lowercased before storage. This means the server can send `White Tower`, `white_tower`, `whitetower`, or any other casing variant and the correct form will be found.
